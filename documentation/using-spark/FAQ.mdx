...
<FAQ.Root className="mt-2xl">
  ...
  <FAQ.Item>
    ...
    />
      <FAQ.Answer>
        If you require tokens that are not part of the Spark theme, you can use our [token extender plugin](?path=/docs/utils-tailwind-plugins-token-extender--docs#tailwind-token-extender-plugin) to achieve this.
        ...
        module.exports = {
          plugins: sparkPlugins.tokenExtender({
            'custom-screen': {
              value: '218px',
              keys: ['screens']
            }
        ...
        <Callout>
          Here, each item included inside **keys** will enable the availability of its corresponding Tailwind classes.

          You can check this [link](https://tailwindcss.com/docs/theme#configuration-reference) for a list of available keys.
        </Callout>
      </FAQ.Answer>

import { Meta, Canvas } from '@storybook/addon-docs'
import { ArgTypes } from '@docs/helpers/ArgTypes'

import { FormField } from '.'

import * as stories from './FormField.stories'

<Meta of={stories} />

# FormField

Provide context to your form elements easily.

## Install

```sh
npm install @spark-ui/form-control
```

## Import

```tsx
import { FormField } from '@spark-ui/form-control'
```

## Props

<ArgTypes
  of={FormField}
  subcomponents={{
    'FormField.Label': FormField.Label,
    'FormField.HelperMessage': FormField.HelperMessage,
    'FormField.StateMessage': FormField.StateMessage,
    'FormField.Control': FormField.Control,
    'FormField.RequiredIndicator': FormField.RequiredIndicator,
  }}
/>

## Usage

By default, all Spark input components use `FormField.Control` behind the scenes, so **you do not need to use it explicitly**. However, if you want to use a custom input component, or if you need more fine-grained control over the Control of a form control, it can be used.

<Canvas of={stories.Default} />

## Required

Set `isRequired` prop to `true` to mark the input as required and display a required indicator. The default indicator is an asterisk `*`.

<Canvas of={stories.Required} />

### Custom required indicator

Configure the required indicator using the `requiredIndicator` prop and the `FormRequiredIndicator` component.

<Canvas of={stories.CustomRequired} />

## State

Use the `state` prop to set the visual validation state. Choose between `error`, `success` and `alert`.

<Canvas of={stories.State} />

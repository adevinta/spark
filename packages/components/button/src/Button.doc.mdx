...
Compose the content of the button using the `Icon` component to add an icon to the left or to the right.
Use `hasOnlyIcon` prop if the button only renders an icon as content. In this particular case you should specify an `aria-label` or `aria-labelledby` attribute to preserve accessibility.
